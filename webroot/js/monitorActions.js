...
(async () => {
  ...
  if (monitor_start) {
    monitor_start.addEventListener('click', () => {
      ...
      exec('/data/adb/modules/nozygisk/bin/zygisk-ptrace64 ctl start')
    })

    monitor_stop.addEventListener('click', () => {
      monitor_status.innerHTML = translations.page.actions.status.exiting

      exec('/data/adb/modules/nozygisk/bin/zygisk-ptrace64 ctl exit')
    })
    ...
    monitor_pause.addEventListener('click', () => {
      ...
      exec('/data/adb/modules/nozygisk/bin/zygisk-ptrace64 ctl stop')
    })
  }
})()